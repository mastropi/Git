...
5) Define the remote repository
	...
	# NOTE: If the remote repository should be stored in GitHub, we should:
	# - first create an empty repository in GitHub
	# - then run the above 'git remote' command by using the URL indicated for the repository in GitHub (HTTPS or SSH link)
	# Ex: git remote add origin git@github.com:mastropi/Git.git
...
105.- Store a local repository in GitHub
----------------------------------------
If we want to create a remote copy of a local repository and store it in GitHub, we should carry out the following steps:
1) Create a new empty repository in GitHub
2) Set the remote repository of our local repository to the newly created GitHub by running in Git Bash from the location of the local repository:
	git remote add origin <remote-URL-as-specified-in-the-newly-created-GitHub-repository>
Ex:
	git remote add origin git@github.com:mastropi/Git.git
3) Push the master branch (and any other branch of interest from the local repository to the remote repository, and set upstream tracking:
	git push --set-upstream origin master
DONE!
******************************************* Additional stuff ****************************************
